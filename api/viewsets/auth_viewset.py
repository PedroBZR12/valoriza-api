...
class AuthViewSet(viewsets.GenericViewSet):
    ...
    def login(self, request):
        ...
        if not user_data:
            return Response({'error': True, 'message': 'Credenciais inválidas.'}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    def register(self, request):
        ...
        if not account_type:
            return Response({'error': True, 'message': 'O campo account_type é obrigatório.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not serializer_class:
            return Response({'error': True,'message': ('account_type inválido. ''Use client, company ou driver.')}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'error': False, 'message': 'Conta criada com sucesso.', 'data': serializer_class(instance).data}, status=status.HTTP_201_CREATED)
    ...
    def logout(self, request):
        ...
        if not refresh_token:
            return Response({'error': True, 'message': 'O refresh token é obrigatório.'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            token = RefreshToken(refresh_token)
            token.blacklist()
        except TokenError:
            return Response({'error': True, 'message': 'Refresh token inválido ou já revogado.'}, status=status.HTTP_400_BAD_REQUEST)

        return Response({'error': False, 'message': 'Logout realizado com sucesso.'},status=status.HTTP_200_OK)